import { behavior, missingBehavior, type FrameworkFileUpload, type FrameworkImageURLResolver } from '@southneuhof/is-vue-framework/adapters/behaviors'
...
export async function defaultImageInputUpload(file: File, directory?: string, onUploadProgress?: (progress: { loaded: number; total: number }) => void) {
  const fileUpload = behavior.fileManager?.uploadFile ?? behavior.imageInput?.fileUpload
...
export function defaultImageURLResolver(payload: Record<string, any> | string) {
  const resolver = behavior.imageInput?.imageURLResolver
