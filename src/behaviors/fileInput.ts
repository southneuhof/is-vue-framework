import { behavior, missingBehavior, type FrameworkFileUpload } from '@southneuhof/is-vue-framework/adapters/behaviors'
...
export async function defaultFileInputUpload(file: File, directory?: string, onUploadProgress?: (progress: { loaded: number; total: number }) => void) {
  const fileUpload = behavior.fileManager?.uploadFile ?? behavior.fileInput?.fileUpload
