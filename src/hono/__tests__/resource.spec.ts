...
const app = new Hono()
  ...
  .post('/roles/create', validator('json', (value) => value as { name: string }), (context) => context.json({ data: context.req.valid('json') }, 201))
...
const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  if (url.pathname.includes('/create')) return new Response(String(init?.body ?? '{}'), { status: 201 })
...
describe('createHonoResourceOperations', () => {
  ...
  it('normalizes data, serializes query and identity, and preserves override-friendly wrappers', async () => {
    ...
    await expect(operations.create({ name: 'Editor' })).resolves.toEqual({ name: 'Editor' })
    await expect(operations.update('r1', { name: 'Updated' })).resolves.toEqual({ data: { id: 'r1' } })
