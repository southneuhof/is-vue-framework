...
import {
  defaultDetailGetData,
  defaultOnDataLoaded,
  getDetailFieldTypes,
} from "@southneuhof/is-vue-framework/behaviors/detail";
...
function formatDetailData(data: Record<string, any>) {
  ...
  Object.keys(data).forEach((field) => {
    ...
    if (props.fieldsDictionary[field])
      view = props.fieldsDictionary[field][data[field]] ?? "-";
...
async function getData() {
  if (!props.data && props.getAPI) {
    await props
      .getData(props.getAPI, props.searchParameters, props.dataID)
      .then((data) => {
    ...
    detailData.value = {
      data: formatDetailData(props.data),
      rawData: props.data,
    };
...
<template>
  <template v-if="!loading">
    <slot
      v-if="$slots['detail-data']"
      name="detail-data"
      v-bind="{ data: detailData?.rawData }"
    />
    <table v-else>
      <tbody>
        <template v-for="(field, index) in fields">
          <tr v-if="field.slice(0, 2) === 'S|'">
            <td colspan="3" class="py-2 print:py-1 print:pb-2">
              <div class="print:bg-[#eeeeee] print:p-2 print:text-[#000000]">
                <p
                  class="text-sm font-bold uppercase print:text-base print:normal-case"
                >
                  {{ field.slice(2) }}
                </p>
              </div>
          ...
          <tr v-else>
            ...
            <td
              :style="{ paddingTop: rowGap, paddingBottom: rowGap }"
              className="px-4 w-[1%] whitespace-nowrap align-top"
            >
              :
            </td>
            <td
            ...
            >
              <component
                ...
                :is="fieldSlots[field]"
                :data="detailData.rawData"
              />
              <slot
                v-else-if="$slots[`detail-${field}`]"
                :name="`detail-${field}`"
                v-bind="{ data: detailData?.rawData, index }"
              ></slot>
              <template v-else-if="detailData?.rawData[field] == null"
                >-</template
              >
              <component
                v-else-if="
                  fieldsType[field]?.type &&
                  detailFieldTypes[fieldsType[field]?.type]
                "
