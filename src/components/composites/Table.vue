...
import {
  defaultTableGetData,
  tableFieldTypes,
} from "@southneuhof/is-vue-framework/behaviors/table";
import {
  defaultTableConfig,
  mergeDefaultConfig,
...
const props = defineProps({
  ...
  fieldsType: {
    type: Object as PropType<Record<string, { type: string; props?: any }>>,
    default: () => ({}),
  },
  fieldsProxy: {
    type: Object as PropType<Record<string, string>>,
  ...
  getData: {
    type: Function as PropType<
      (
        getAPI: string,
        searchParameters?: Record<string, number | string | undefined>,
      ) => Promise<{
  ...
  fieldsAlign: {
    type: Object as PropType<Record<string, "start" | "center" | "end">>,
    required: false,
  },
  ...
  rowClass: {
    type: Function as PropType<
      (data: Record<string, any>, index: number) => string
    >,
    default: () => "",
  },
...
const fieldsAlias = mergeDefaultConfig(
  defaultTableConfig.fieldsAlias,
  props.fieldsAlias,
) || {};
const fieldsProxy = mergeDefaultConfig(
  defaultTableConfig.fieldsProxy,
  props.fieldsProxy,
) || {};
const fieldsType = mergeDefaultConfig(
  defaultTableConfig.fieldsType,
  props.fieldsType,
) || {};
const fieldsClass = mergeDefaultConfig(
  defaultTableConfig.fieldsClass,
  props.fieldsClass,
) || {};
const fieldsHeaderClass = mergeDefaultConfig(
  defaultTableConfig.fieldsHeaderClass,
  props.fieldsHeaderClass,
) || {};
const fieldsParse = mergeDefaultConfig(
  defaultTableConfig.fieldsParse,
  props.fieldsParse,
) || {};
const fieldsAlign = mergeDefaultConfig(
  defaultTableConfig.fieldsAlign,
  props.fieldsAlign,
) || {};

const localsearchParameters = ref<Record<string, any>>({
  page: "1",
  limit: props.draggable ? 9999 : props.limitSet[0],
});
...
function formatTableData(data: Record<string, any>[]) {
  ...
  data.forEach((row, index) => {
    ...
    Object.keys(row).forEach((field) => {
      ...
      if (props.fieldsDictionary[field])
        view = props.fieldsDictionary[field][row[field]] ?? "-";
...
function handleFieldFilter(field: string) {
  if (localsearchParameters.value.sort_by === field) {
    if (localsearchParameters.value.sort === "asc")
...
async function loadData() {
  loading.value = true;
  if (props.getAPI) {
    const { data, total, totalPage } = await props.getData(
      props.getAPI!,
      localsearchParameters.value,
    );
    ...
    tableData.value = {
      ...props.data,
      data: formatTableData(props.data),
      rawData: props.data,
    };
...
function onDragChange(event: any) {
  ...
  const newRaw = (tableData.value.data || []).map(
    (it: any, i: number) => it?.rawData ?? tableData.value.rawData[i],
  );
  tableData.value = {
    ...tableData.value,
    rawData: newRaw,
...
function onMouseMove(e: MouseEvent) {
  ...
  const initialWidth =
    initialWidths.value[resizingField.value] ||
  ...
  <div class="flex flex-col gap-4">
    <div class="relative block w-full overflow-x-auto rounded-xl">
      <template v-if="tableData.data.length && !loading">
        <slot
          v-if="$slots['list-data']"
          name="list-data"
          v-bind="{ data: tableData.rawData, props }"
        />
        <table v-else class="w-full table-auto">
          <slot
            v-if="$slots['list-thead']"
            name="list-thead"
            v-bind="{ data: tableData.rawData, props }"
          ></slot>
          <thead v-else>
            <tr>
              ...
              >
                <button
                  ...
                  variant="standard"
                >
                  <div class="font-semibold print:text-black">
                    {{ fieldsAlias[field] ?? field }}
                  </div>
                  <Icon
                    v-if="localsearchParameters.sort_by === field"
                    size="sm"
                    :name="
                      localsearchParameters.sort === 'asc'
                        ? 'arrow-up'
                        : 'arrow-down'
                    "
                  />
                </button>
                <div v-else class="font-semibold print:text-black">
                  {{ fieldsAlias[field] ?? field }}
                </div>
                <!-- Resizer handle -->
                <div
                  class="absolute right-0 top-0 z-50 flex h-full w-2 cursor-col-resize items-center justify-center"
                  @mousedown.prevent="startResize($event, field)"
                >
                  <div
                    class="pointer-events-none h-full w-px transition-colors duration-150 bg-outline/[24%]"
                    :class="
                      resizingField === field
                        ? 'bg-on-primary dark:bg-on-surface'
                        : ''
                    "
                  ></div>
                </div>
          ...
          <template v-else>
            ...
            >
              <template #item="{ element: item, index }">
                <tr
                  ...
                  :class="[
                    {
                      'overlay after:pointer-events-none after:bg-primary/[8%]':
                        hoverEffect,
                      'bg-surface-container-highest/[12%]':
                ...
                >
                  <td
                    v-if="$slots['list-rowActions'] && tableData.rawData[index]"
                    class="p-3"
                  >
                    <div class="flex flex-row items-center gap-4">
                      <Button
                        ...
                        @click="
                          () =>
                            rowExpandActiveIndex === index
                              ? (rowExpandActiveIndex = null)
                              : (rowExpandActiveIndex = index)
                        "
                        ><Icon
                          :name="
                      ...
                      /></Button>
                      <slot
                        name="list-rowActions"
                        v-bind="{ data: tableData.rawData[index], index }"
                      />
                    </div>
                  </td>
                  <td
                    ...
                    class="p-2"
                    :class="
                      fieldsAlignClassMap[fieldsAlign[field] ?? 'start'].text
                    "
                  >
                    <component
                      v-if="fieldSlots[field]"
                      :is="fieldSlots[field]"
                      :data="{ data: item }"
                    />
                    <slot
                      v-else-if="$slots[`list-${field}`]"
                      :name="`list-${field}`"
                      v-bind="{ data: item.rawData, index }"
                    />
                    <p v-else-if="tableData?.rawData[index]?.[field] == null">
                      -
                    </p>
                    <component
                      v-else-if="
                        fieldsType[field]?.type &&
                        tableFieldTypes[fieldsType[field]?.type]
                      "
                      ...
                      :index="index"
                    />
                    <p v-else :class="fieldsClass[field]">
                      {{ item[field] ?? "-" }}
                    </p>
                  </td>
                </tr>
            ...
            <tr v-if="$slots['list-suffix-row']">
              <slot
                name="list-suffix-row"
                v-bind="{ data: tableData.rawData }"
              ></slot>
            </tr>
          </template>
        </table>
      ...
      <div
        v-else
        class="mt-2 flex items-center justify-center text-muted print:items-start print:justify-start"
      >
        Tidak ada data
      </div>
    </div>
    <Pagination
      v-if="props.paginated && !props.draggable"
      :limitSet="limitSet"
      v-model="localsearchParameters"
      :dataInfo="dataInfo as any"
    >
      <template
        v-if="$slots['pagination-lengthControl']"
