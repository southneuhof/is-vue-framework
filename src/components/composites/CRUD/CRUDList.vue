...
import {
  ...
  defaultOnDragChange,
  onDelete,
} from "@southneuhof/is-vue-framework/behaviors/crudList";
...
const listConfig = ref<ListConfig>(
  buildListConfig(props.config, {
    ...
    fieldsType: defaultTableConfig.fieldsType as Record<
      string,
    ...
    onDragChange: (event: any) =>
      defaultOnDragChange(
        props.config.view?.list?.getAPI ||
          props.config.view?.getAPI ||
...
const defaultActiveFilterInput = {
  type: "radio",
  props: {
    required: true,
    data: [
...
const filterProps = computed(() => ({
  fields: listConfig.value.filter?.fields,
  fieldsAlias:
    listConfig.value.filter?.fieldsAlias || listConfig.value.fieldsAlias,
  inputConfig: {
    active:
      listConfig.value.filter?.inputConfig?.active || defaultActiveFilterInput,
...
function handleVisibleFields(field: string, enable: boolean) {
  ...
  if (enable) {
    ...
    listConfig.value.fields = listConfig.value.fields.filter(
      (item) => item !== field,
    );
}
  ...
  <div class="flex h-full flex-col gap-2 overflow-hidden">
    <Card class="flex-shrink-0">
      <div class="flex flex-col gap-4">
        <div
          class="flex w-full flex-row flex-wrap items-center justify-between gap-4"
        >
          <slot
            v-if="$slots['list-view-title']"
            name="list-view-title"
            v-bind="{ title: config.title }"
          />
          <div v-else class="flex flex-row items-center gap-4">
            <div class="flex flex-row items-center gap-4">
              <div class="flex flex-row items-center gap-6">
                <h1 class="min-w-max text-xl font-semibold">
                  {{ config.title || $route.meta.title }}
                </h1>
                <SearchBox v-model="searchParameters.search" />
              </div>
              <slot
                v-if="$slots['list-view-header-action']"
                name="list-view-header-action"
              />
            </div>
          </div>
          <div
            ...
            :class="
              (config.actions?.create ?? true) && !$slots.createButton
                ? 'divide-x divide-outline/[24%] '
                : ''
            "
          >
              ...
              <div class="flex items-center justify-between gap-4">
                <div class="flex flex-row gap-4">
                  <Tooltip>
                    <template #trigger>
                      <Popover
                        v-if="config.view?.list?.filter?.fields?.length"
                        :ignore="['#form-lookup']"
                      >
                        <template #trigger>
                          <Button kind="icon">
                            ...
                            <Button
                              @click="
                                () => {
                                  ...
                                  keyManager().triggerChange(
                                    `sys_${config.name}_name`,
                                  );
                                }
                              "
                            >
                      ...
                      <Dialog :title="`Atur Kolom Tabel ${config.title}`">
                        ...
                        <template #content>
                          <div class="grid grid-cols-3 gap-8">
                            <div
                              v-for="field in Object.keys(visibleFields)"
                              class="flex max-h-fit flex-row items-center gap-4"
                            >
                              <Switch
                                v-model="visibleFields[field]"
                                :onDeactive="
                                  () => handleVisibleFields(field, false)
                                "
                                :onActive="
                                  () => handleVisibleFields(field, true)
                                "
                              />
                              <div>
                                {{ listConfig.fieldsAlias?.[field] || field }}
                              </div>
                            </div>
                          </div>
                        </template>
            ...
            >
              <slot
                v-if="$slots['list-createButton']"
                name="list-createButton"
                v-bind="{ listConfig }"
              >
              </slot>
              <Button
              ...
              <div class="min-h-0 flex-1 overflow-auto">
                  ...
                  :onDataLoaded="
                    (data: Array<any>) => {
                      visibleFields = listConfig.toggleableFields
                        ? Object.fromEntries(
                            listConfig.toggleableFields.map((field) => [
                              field,
                              (listConfig.fields ?? []).includes(field),
                            ]),
                          )
                        : data[0]
                          ? Object.fromEntries(
                              Object.keys(data[0])
                                .filter(
                                  (field) => !!listConfig.fieldsAlias?.[field],
                                )
                                .map((field) => [
                                  field,
                                  (listConfig.fields ?? []).includes(field),
                ...
                >
                  <template
                    ...
                    v-slot:[String(slotname)]="data"
                  >
                    <slot
                      v-if="slotname.slice(0, 5) === 'list-'"
                      :name="slotname"
                      v-bind="data as any"
                    ></slot>
                  </template>
                  <template #list-rowActions="{ data }">
                    ...
                    ></slot>
                    <div
                      v-else-if="data?.[listConfig.uid!]"
                      class="flex flex-row items-center gap-2"
                    >
                      <slot
                        v-if="$slots['list-rowActions-detail']"
                        name="list-rowActions-detail"
                        v-bind="{ data, permissions, config }"
                      />
                      <Button
                        v-else-if="
                          (config.actions?.detail ?? true) && permissions.detail
                        "
                        color="info"
                        kind="icon"
                        @click="
                          () =>
                            router.push({
                              ...
                              query: {
                                ...route.query,
                                [`${config.name}_view`]: 'detail',
                                [`${config.name}_id`]: data[listConfig.uid!],
                              },
                            })
                        "
                      >
                      ...
                      <slot
                        v-if="$slots['list-rowActions-update']"
                        name="list-rowActions-update"
                        v-bind="{ data, permissions, config }"
                      />
                      <Button
                        v-else-if="
                          (config.actions?.update ?? true) &&
                      ...
                      <slot
                        v-if="$slots['list-rowActions-delete']"
                        name="list-rowActions-delete"
                        v-bind="{ data, permissions, config }"
                      />
                      <ConfirmationDialog
                        v-else-if="
                        ...
                        :onConfirm="
                          async () =>
                            await onDelete(config.name, data[listConfig.uid!])
                        "
                        :onSuccess="
                          () => {
                            toast.success(
                              `${config.title || config.name} berhasil dihapus`,
                            );
                      ...
                      <slot
                        v-if="$slots['list-rowAdditionalActions']"
                        name="list-rowAdditionalActions"
                        v-bind="{ data, permissions, config }"
                      ></slot>
                    </div>
                  </template>
