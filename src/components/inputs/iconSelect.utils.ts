...
function collectIconClassesFromRule(rule: CSSRule, out: Set<string>) {
  ...
  if (typeof asStyleRule.selectorText === "string") {
    ...
    while (
      (match = ICON_SELECTOR_REGEX.exec(asStyleRule.selectorText)) !== null
    ) {
      out.add(match[1]);
    }
  }
...
export function extractRemixiconClassNamesFromStyleSheets(
...
export function filterRemixiconClassNames(
  icons: string[],
  query: string,
): string[] {
...
export function normalizeRemixiconClass(
  value: unknown,
): string {
